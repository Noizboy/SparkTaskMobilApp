export default {
  // Common
  order: 'Orden',
  cancel: 'Cancelar',
  save: 'Guardar',
  status: 'Estado',
  tasks: 'tareas',
  duration: 'Duración',
  photos: 'Fotos',
  before: 'Antes',
  after: 'Después',
  add: 'Agregar',
  date: 'Fecha',
  time: 'Hora',
  sections: 'Secciones',
  addOns: 'Extras',
  skip: 'Omitir',
  resume: 'Reanudar',

  // Login
  loginTitle: 'Spark Task',
  loginTagline: 'Gestión de limpieza profesional',
  welcomeBack: 'Bienvenido',
  signInSubtitle: 'Inicia sesión para acceder a tu cuenta',
  emailAddress: 'Correo electrónico',
  password: 'Contraseña',
  forgotPassword: '¿Olvidaste tu contraseña?',
  signIn: 'Iniciar Sesión',
  invalidCredentials: 'Correo o contraseña incorrectos',
  demoCredentials: 'Credenciales de prueba',

  // Onboarding
  onboardingTitle1: 'Bienvenido a Spark Task',
  onboardingSub1: 'Tu herramienta profesional para gestionar limpiezas. Organizado, eficiente y sin complicaciones.',
  onboardingTitle2: 'Controla tu Agenda',
  onboardingSub2: 'Visualiza todos tus trabajos de un vistazo. No pierdas ninguna cita con el calendario inteligente.',
  onboardingTitle3: 'Sigue el Checklist',
  onboardingSub3: 'Completa cada tarea paso a paso. Las listas por sección te mantienen organizado.',
  onboardingTitle4: 'Documenta tu Trabajo',
  onboardingSub4: 'Toma fotos del antes y después de cada sección. Genera confianza con tus clientes.',
  getStarted: 'Comenzar',
  next: 'Siguiente',

  // Home
  welcome: 'Hola',
  inProgress: 'En Curso',
  upcomingJobs: 'Próximos Trabajos',
  completed: 'Completado',
  viewAll: 'Ver Todo',
  noUpcomingJobs: 'No hay trabajos próximos',
  noUpcomingJobsSub: 'Ya completaste todos los trabajos asignados para hoy.',

  // Calendar
  calendar: 'Calendario',
  todaysJobs: 'Trabajos de Hoy',
  jobsOn: 'Trabajos del',
  total: 'Total',
  noJobsScheduled: 'Sin trabajos programados para este día',
  noJobsScheduledSub: 'Consulta más tarde o revisa los trabajos disponibles.',

  // Hub
  hub: 'Resumen',
  yourWorkOverview: 'Tu actividad laboral',
  clockedIn: 'En turno',
  since: 'desde las',
  nextJob: 'Próximo Trabajo',
  thisWeek: 'Esta Semana',
  thisMonth: 'Este Mes',
  allTime: 'Histórico',
  jobsDone: 'Realizados',
  hours: 'Horas',
  avgPerJob: 'Prom. / Trabajo',
  streak: 'Racha',
  completeRate: 'Completado',
  vsLastWeek: 'vs Semana Anterior',
  standings: 'Reputación',
  atRisk: 'En Riesgo',
  fair: 'Regular',
  good: 'Bueno',
  great: 'Excelente',
  fantastic: 'Fantástico',
  howToReach: 'Cómo llegar a',
  standingTip: 'Sigue recibiendo buenas reseñas. Tu reputación se basa en',
  ratings: 'calificaciones con un promedio de',
  mostSkippedSections: 'Secciones Más Omitidas',
  clockHistory: 'Historial de Turnos',
  noClockEntries: 'Sin registros aún',
  noClockEntriesSub: 'Completa un trabajo para ver tu historial',
  today: 'Hoy',
  yesterday: 'Ayer',

  // Notifications
  notifications: 'Notificaciones',
  markAllAsRead: 'Marcar todo como leído',
  unreadNotification: 'notificación sin leer',
  unreadNotifications: 'notificaciones sin leer',
  noNotifications: 'Aún no hay notificaciones',
  allCaughtUp: '¡Estás al día! Vuelve más tarde.',
  notifAssignedTitle: '¡Tienes un nuevo trabajo!',
  notifAssignedMessage: 'La orden #{{orderNumber}} ha sido agregada a tu horario.',
  notifAssignedMessageWithDate: 'La orden #{{orderNumber}} está programada para el {{date}} a las {{time}}.',
  notifUnassignedTitle: 'Trabajo Eliminado',
  notifUnassignedMessage: 'La orden #{{orderNumber}} ha sido eliminada de tu horario.',
  justNow: 'Ahora mismo',

  // Profile
  profile: 'Perfil',
  account: 'Cuenta',
  email: 'Correo',
  phone: 'Teléfono',
  language: 'Idioma',
  security: 'Seguridad',
  changePassword: 'Cambiar Contraseña',
  currentPassword: 'Contraseña Actual',
  newPassword: 'Nueva Contraseña',
  confirmPassword: 'Confirmar Contraseña',
  updatePassword: 'Actualizar Contraseña',
  passwordUpdated: 'Contraseña actualizada correctamente.',
  passwordsNoMatch: 'Las contraseñas no coinciden.',
  passwordTooShort: 'La contraseña debe tener al menos 6 caracteres.',
  about: 'Información',
  appVersion: 'Versión',
  signOut: 'Cerrar Sesión',
  signOutConfirm: '¿Seguro que deseas cerrar sesión?',
  permissionNeeded: 'Permiso necesario',
  allowPhotos: 'Permite el acceso a tus fotos.',
  success: 'Listo',
  error: 'Error',
  selectLanguage: 'Seleccionar Idioma',
  help: 'Ayuda',
  contactUs: 'Contáctanos',
  howTo: 'Cómo Funciona',
  howToSub: 'Conoce el servicio',
  faq: 'Preguntas Frecuentes',
  faqSub: 'Dudas comunes',
  helpCenter: 'Centro de Ayuda',
  helpCenterSub: 'Obtén soporte',
  legal: 'Legal',
  termsOfService: 'Términos de Servicio',
  privacyPolicy: 'Política de Privacidad',

  // Job Info
  startJob: 'Iniciar Trabajo',
  goal: 'Objetivo',
  specialInstructions: 'Notas del Cliente',
  accessInfo: 'Información de Acceso',
  availableAddOns: 'Add-ons',
  jobInProgress: 'Trabajo en curso',
  mustCompleteFirst: 'Debes completar la Orden #{{orderNumber}} antes de iniciar otro trabajo.',

  // Checklist
  overallProgress: 'Progreso General',
  tasksDone: 'Tareas listas',
  pending: 'Pendiente',
  swipeToComplete: 'Desliza para Completar',
  completed_excl: '¡Completado!',
  noSectionsInCategory: 'No hay secciones en esta categoría.',
  skipSection: 'Omitir Sección',
  skipThis: 'Omitir esta sección',
  selectOrTypeReason: 'Selecciona o escribe un motivo',
  orTypeCustomReason: 'O escribe un motivo...',
  clientRequest: 'Pedido del cliente',
  alreadyClean: 'Ya está limpio',
  noAccess: 'Sin acceso',
  outOfTime: 'Sin tiempo',
  other: 'Otro',
  skipped: 'Omitido',
  estimatedTime: 'Tiempo estimado',
  addPhoto: 'Agregar Foto',
  chooseSource: 'Seleccionar origen',
  camera: 'Cámara',
  gallery: 'Galería',
  permissionNeededCamera: 'Se necesita acceso a la cámara o galería.',
  swipeRightToCancel: 'Desliza a la derecha para cancelar',
  cancelJob: 'Cancelar Trabajo',
  canceled: 'Cancelado',
  selected: 'seleccionados',

  // Order Details
  orderDetails: 'Detalles de la Orden',

  // Day Jobs
  jobScheduled: 'trabajo programado',
  jobsScheduled: 'trabajos programados',
  noJobsForDay: 'Sin trabajos para este día',
  nothingScheduledHere: 'Nada programado.',

  // All Upcoming/Completed
  completedJobs: 'Trabajos Completados',
  jobCompleted_count: 'trabajo completado',
  jobsCompleted_count: 'trabajos completados',
  noCompletedJobs: 'Aún no hay trabajos completados',
  completedJobsWillAppear: 'Los trabajos completados aparecerán aquí.',

  // Job Completed
  greatJob: '¡Buen trabajo!',
  youCompletedOrder: 'Completaste la Orden',
  inTime: 'en',
  viewOrderDetails: 'Ver Detalles de la Orden',

  // Photo Gallery
  photosLabel: 'Fotos',

  // InProgressJobCard
  scheduledFor: 'Programado para',
  overallProgressLabel: 'Progreso General:',
  ofTasksCompleted: 'de {{total}} tareas completadas',
  timeElapsed: 'Tiempo transcurrido:',

  // StatusBadge
  upcoming: 'Próximo',
  inProgressStatus: 'En Curso',
  completedStatus: 'Completado',

  // Offline / Sync Banner
  syncOffline: 'Sin conexión · cambios guardados localmente',
  syncSyncing: 'Sincronizando cambios pendientes...',
  syncDone: 'Cambios sincronizados',
} as const;
