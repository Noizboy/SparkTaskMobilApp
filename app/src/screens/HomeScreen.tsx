...
export function HomeScreen() {
  ...
  const { jobs, jobsLoaded, unreadCount, currentUser } = useApp();
  const { t } = useLanguage();
  ...
  // Include all non-canceled jobs — date is already YYYY-MM-DD, use as-is (no Date parse to avoid UTC offset)
  const markedDates = jobs.filter((j) => j.status !== 'canceled').map((j) => j.date);
