...
import { fetchJobs, apiToggleTodo, apiToggleAddOn, apiMarkSectionDone, apiMarkSectionUndone, apiUpdateStatus, apiUpdateOrder, apiUploadPhoto, apiDeletePhoto, apiGetOrder, apiFetchNotifications, apiMarkNotificationRead, apiMarkAllNotificationsRead } from '../services/api';
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  // Register Expo push token whenever a real JWT is available
  ...
  const loadNotifications = useCallback(async () => {
    try {
      ...
      if (!token || token === 'true') return; // demo/offline users have no real JWT
      const notifs = await apiFetchNotifications(token);
      setNotifications(
        notifs.map((n) => ({
          id: n.id,
      ...
      console.warn('[Notifications] Failed to load from API:', err);
    }
  }, []);
  ...
  const markAsRead = (id: string) => {
    ...
    storage.get(AUTH_CONFIG.STORAGE_KEYS.AUTH_TOKEN).then((token) => {
      if (token && token !== 'true') apiMarkNotificationRead(token, id).catch(() => {});
    });
  };
  ...
  const markAllRead = () => {
    ...
    storage.get(AUTH_CONFIG.STORAGE_KEYS.AUTH_TOKEN).then((token) => {
      if (token && token !== 'true') apiMarkAllNotificationsRead(token).catch(() => {});
    });
  };
  ...
  useEffect(() => {
    const initAuth = async () => {
      ...
      if (saved && saved !== 'true') {
        // Real JWT — restore token for push registration
        setAuthToken(saved);
  ...
  useSSE({
    ...
    onOrderAssigned: (order) => {
      ...
      // Reload notifications from DB to get the persisted record with correct id/time
      // Fall back to an optimistic local notification if API is unreachable
      loadNotifications().catch(() => {
        setNotifications((prev) => [
          {
            id: `assigned-${job.id}-${Date.now()}`,
            type: 'assigned' as AppNotification['type'],
            title: t('notifAssignedTitle'),
            message: t('notifAssignedMessage', { orderNumber: job.orderNumber }),
    ...
    onOrderUnassigned: ({ id, orderNumber }) => {
      ...
      // Reload notifications from DB to get the persisted record
      loadNotifications().catch(() => {
        setNotifications((prev) => [
          {
            id: `unassigned-${id}-${Date.now()}`,
            type: 'removed' as AppNotification['type'],
            title: t('notifUnassignedTitle'),
            message: t('notifUnassignedMessage', { orderNumber }),
            time: new Date().toISOString(),
            isRead: false,
          },
          ...prev,
  ...
  const handleLogin = async (user: CurrentUser, token?: string) => {
    const resolvedToken = token || 'true';
    await storage.set(AUTH_CONFIG.STORAGE_KEYS.AUTH_TOKEN, resolvedToken);
    await storage.setJSON('currentUser', user);
    setCurrentUser(user);
    setIsAuthenticated(true);
    // Only set authToken for real JWTs so push registration is triggered
    if (resolvedToken !== 'true') setAuthToken(resolvedToken);
  ...
  const startJob = async (jobId: string): Promise<void> => {
    ...
    try {
      await apiUpdateStatus(jobId, 'in-progress', {
        startedAt,
        clientUpdatedAt: targetJob?.updatedAt ?? undefined,
      });
    } catch (err: any) {
      if (err?.message === 'conflict') {
        // Another user modified this order — revert and show alert
        saveJobs(previous);
        Alert.alert('Conflict', 'This job was updated by another team member. Please refresh.');
        return;
      }
  ...
  const completeJob = (jobId: string, skipReasons?: Record<string, string>) => {
    ...
    if (capturedJob) {
      ...
      apiUpdateOrder(jobId, jobSnapshot, { clientUpdatedAt: jobSnapshot.updatedAt }).catch(async (err) => {
        if (err.message === 'conflict') {
          // Check server state — if already completed, no action needed (both cleaners finished)
          try {
            const serverJob = await apiGetOrder(jobId);
            if (serverJob.status === 'completed') {
              // Silent refresh — the order is already done on the server, nothing to alert
            ...
            // If we can't fetch the server state, fall through to show the alert
          }
